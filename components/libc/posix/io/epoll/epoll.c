...
static int epoll_rdlist_add(struct rt_fd_list *fdl, int fd, rt_uint32_t revents)
{
...
static int epoll_wqueue_callback(struct rt_wqueue_node *wait, void *key)
{
    ...
    if (fdlist->revents)
    {
        epoll_rdlist_add(fdlist, fdlist->fd, (rt_ubase_t)key);
    }
...
static void epoll_ctl_install(struct rt_fd_list *fdlist, int fd, struct rt_eventpoll *ep)
{
    rt_uint32_t mask = 0;
    ...
    if (mask & fdlist->revents)
    {
        epoll_rdlist_add(fdlist, fd, mask);
    }
}
...
static int epoll_ctl_add(struct dfs_file *df, int fd, struct epoll_event *event)
{
    ...
    if (df->vnode->data)
    {
        ...
        if (fdlist)
        {
            ...
            epoll_ctl_install(fdlist, fd, ep);
        }
        else
        {
            ret = -ENOMEM;
    ...
    if (df->vnode->data)
    {
        ...
        while (fdlist->next != RT_NULL)
        {
            if (fdlist->next->fd == fd)
            {
                ...
                epoll_ctl_install(fdlist->next, fd, ep);
                break;
            }
